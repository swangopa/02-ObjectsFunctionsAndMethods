...
def main():
    ...
    even_better_draw_circles(rg.Point(100, 50), 15,5,5,'blue')
    even_better_draw_circles(rg.Point(-200, 0), 25,10,2,'red')
    window.update()
...
def even_better_draw_circles(point,radius,count,thick,color):
    ...
    turtle = rg.SimpleTurtle()
    turtle.pen.thickness=thick
    turtle.pen.color=color
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(1, count+1):  # k becomes 1, 2, 3, ... 10
